...
function cmb_dinamis_investasi_user($name,$table,$where,$field,$pk,$selected=null,$order=null){
    ...
    $ci->db->where('tbl_user_level.nama_level', $where);
	$ci->db->select('*');
	$ci->db->from($table);
	$ci->db->join('tbl_user_level', 'tbl_user_level.id_user_level = tbl_user.id_user_level');
...
function cmb_dinamis_user($name,$table,$where,$field,$pk,$selected=null,$order=null){
    $ci = get_instance();
    $cmb = "<select name='$name' class='form-control'>";
    if($order){
        $ci->db->order_by($field,$order);
