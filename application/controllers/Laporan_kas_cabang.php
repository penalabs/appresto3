...
class Laporan_kas_cabang extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        
        $this->load->model('Laporan_kas_cabang_model');
    ...
    {
        $this->template->load('template','laporan_kas_cabang/biaya_operasional_cabang_list');
    } 
    
    public function json() {
        header('Content-Type: application/json');
        echo $this->Laporan_kas_cabang_model->json();
    }

    public function read($id) 
    {
        $row = $this->Laporan_kas_cabang_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'kas_id' => $row->kas_id,
            ...
            $this->template->load('template','laporan_kas_cabang/biaya_operasional_cabang_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('laporan_kas_cabang'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('laporan_kas_cabang/create_action'),
        ...
        $this->template->load('template','laporan_kas_cabang/biaya_operasional_cabang_form', $data);
    }
    ...
    public function create_action() 
    {
		...
		'kas_id' => $this->input->post('kas_id',TRUE),
            ...
            $this->Laporan_kas_cabang_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('laporan_kas_cabang/update_action'),
    ...
    public function update_action() 
    {
		...
		'kas_id' => $this->input->post('kas_id',TRUE),
            ...
            $this->Laporan_kas_cabang_model->update($this->input->post('biaya_operasional_id', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Laporan_kas_cabang_model->delete($id);
	...
	foreach ($this->Laporan_kas_cabang_model->get_all() as $data) {
            $kolombody = 0;
    ...
    public function word()
    {
        ...
        $data = array(
            'biaya_operasional_cabang_data' => $this->Laporan_kas_cabang_model->get_all(),
            'start' => 0
        );
        
        $this->load->view('laporan_kas_cabang/biaya_operasional_cabang_doc',$data);
    }

}

/* End of file Laporan_kas_cabang.php */
/* Location: ./application/controllers/Laporan_kas_cabang.php */
